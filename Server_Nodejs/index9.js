...
server.listen( process.env.PORT || 3100, console.log('server_start_port_3500-de-lang-nghe-socket.io-send-image-from-Web-to-App'));
...
var express = require('express');
var app2 = express();;
app2.listen( process.env.PORT || 1400, console.log('app2-start-port 1400-upload-image-From-app-to-web'))
...
app2.post('/reactNative/Upload', (req, res) => {
    req.fields;
    var x = req.files;
    var path = req.files.lan.path;
    var pathNew = path + '.jpg';
    console.log('name:::', path)
    fs.rename(path, pathNew, err => {
        res.send(pathNew);
    });
...
server1.listen( process.env.PORT || 2800, console.log('app3 server_start_port_2800-de--lang-nghe-socket.io-send-image-from-Web-to-App'));
var io1 = require('socket.io')(server1);


/*
var io2 = require('socket.io')(3100);
...
/*
app3.post('/React/upfile', urlencodedParser, (req, res) => {
...
app3.post('/React/upfile', urlencodedParser, (req, res, next) => {
    //  console.log(req);
    let files = req.files;
    console.log('files::::', files);
    let image = req.files.file;
    //   let imageFile = req.files.file;
    console.log('req.imageFile imageFile:::::', image);
    var name = req.body.filename;
    var nameNew = Date.now() + '_' + name
    //image.mv(`${__dirname}/public/upload/${req.body.filename}.jpg`, function (err) {
    image.mv(`${__dirname}/public/upload/${nameNew}.jpg`, function (err) {

        if (err) {
            return res.status(500).send(err);
        }
        res.json({ file: `upload/${nameNew}.jpg` });
        //  res.json({ file: `public/upload/${nameNew}.jpg` });
        // res.json({ file: `public/${req.body.filename}.jpg` });
    });

    // fs.rename(name,Date.now() + '_' + name, err => {console.log('err nhe')});
...
app3.post('/React/upload', upload, function (req, res) {
...
app3.post('/ReactNative/Upload', (req, res) => {
...
// app.listen(8000, console.log('server-connect-port-8000'));
  var server5 = require('http').Server(app5);

server5.listen(8500, console.log('server-connect-port-8500'));
...
///////////////////////////
//////////////////////////
app3.get('/stream', (req, res) => {
    res.render('stream');
});

app3.get('/stream1', (req, res) => {
    res.render('stream1');
})
...
app3.get('/stream2', (req, res) => {
    res.render('stream2');
})

app3.get('/stream2/call', (req, res) => {
    res.render('call');
})

app3.get('/stream3', (req, res) => {
    res.render('stream3');
})
...
io1.on('connection', (socket) => {
    ...
    */
    ...
    socket.on('NGUOI_DUNG_DANG_KY_STREAM_PREE', dataStream => {
        ...
        io1.sockets.emit('DANH-SACH-USER-PEER', ArrPeerId);
    ...
    socket.on('client-call-video', UserCallUserAnswn => {
        console.log('UserCallUserAnswn:::::', UserCallUserAnswn);
        // UserCallUserAnswn = { UserCall: ..., UserCall:  }
        var UserCall = UserCallUserAnswn.UserCall;
        var UserAnswn = UserCallUserAnswn.UserAnswn;

        //    { UserSocketId: '1d5j49CQY2U8-sweAAAJmanh',
        //       Username: 'manh',
        //        peerId: 'bmmyw9o1bb600000' }

        //tim peer nguoi nhan video tim peer UserAnswn;
        var ArrPeerAnswn = [];
        ArraySocketIdUsername.map(function (value, index) {
            var Username = value.Username;
            var PeerId = value.peerId;
            if (Username.indexOf(UserAnswn) > -1) {
                ArrPeerAnswn.push(PeerId);
            }
        })
        //chi emit toi nguoi call tra lai peerAnswn cho nguoi goi de goi
        socket.emit('server-traVe-peer-nguoiNhan-videoCall', ArrPeerAnswn);// emit mang peer nhan
    ...
    var soOnMess = 0;
    var ArrMessSendServer = [];
    socket.on('client-yeucau-read-ArrMessSendServer', dataYeuCauMs => {
        // var data= ArrMessSendServer;
        // var x = [{ data: ArrMessSendServer }];
        //doc ArrMessSendServer
        console.log('dataYeuCauMs:::', dataYeuCauMs);
        var UserWeb = dataYeuCauMs.UsernameNguoiSend;
        var UserApp = dataYeuCauMs.UsernameNguoiNhan;
        ...
        fs.readFile(path, 'UTF-8', (err, data) => {
            ...
            if (err) {
                console.log('da co error say ra xem lai path');
            } else {
                var SaveDataMessengerApp1 = data;
                ArrMessSendServer = JSON.parse(SaveDataMessengerApp1);
                console.log('ArrMessSendServer ArrMessSendServer00000000000', ArrMessSendServer);
            }
        });
       // ArrMessSendServer = ArrMessSendServer;
    ...
    socket.on('client-send-messenger', dataMessenger => {
        ...
        var ArrPeerIdUserNhan = []; //lay cai Peer nguoi se nhan video cal tra lai cho nguoi muon chat chi can socket.emit('') // la tra lai nguoi gui tin nhan
        ArraySocketIdUsername.map(function (socketIdUsername, index) {
            if (socketIdUsername.UserSocketId.indexOf(dataMessenger.UsernameNguoiNhan) > -1) {
                ...
                var PeerId = socketIdUsername.peerId;
                ArraySocketIdUserNhan.push(SocketId);
                ArrPeerIdUserNhan.push(PeerId);
            }
        });
        ...
        fs.readFile(path, 'UTF-8', (err, data) => {
            ...
            if (err) {
                ...
                var SaveDataMessengerApp1 = data;
                ArrMessSendServer = JSON.parse(SaveDataMessengerApp1);
                console.log('ArrMessSendServer ArrMessSendServer000000000001111111111111', ArrMessSendServer);
            }
        });

        console.log(' ArrMessSendServer::ArrMessSendServer ArrMessSendServer 1111111111111111111:', ArrMessSendServer);
        ...
        if (ArrMessSendServer == false) {
            ...
            dataMessenger.map(async (dataEmit, index) => {
                ...
                console.log(' ArrMessSendServer::ArrMessSendServer ArrMessSendServer:222222222222222222', ArrMessSendServer);
    ...
    var NameStatusPubic = 'StatusPubic.docx';
    //   var NameUserSendUserItem = UserApp + UserWeb + "ChatUsername.docx";
    //   var NameUserSendUserItem1 = UserWeb + UserApp + "ChatUsername.docx";
    var name = NameStatusPubic;

    var path = __dirname + "/public/StatusPublic/" + name;
    var ArrStatusSendServer = [];
    fs.readFile(path, 'UTF-8', (err, data) => {
       // console.log(data);
        if (err) {
            ArrStatusSendServer = [];
        } else {
            ArrStatusSendServer = JSON.parse(data);
           //  console.log('ArrStatusSendServer trong socket.on(connect)00000000000000',ArrStatusSendServer);
        }
    ...
    socket.on('client-share-status-public-congKhai', (dataStatus) => {
        ...
        var NameStatusPubic = 'StatusPubic.docx';
        var nameStatusUser = dataStatus.User + ".docx";

        //   var NameUserSendUserItem = UserApp + UserWeb + "ChatUsername.docx";
        //   var NameUserSendUserItem1 = UserWeb + UserApp + "ChatUsername.docx";
        var name = NameStatusPubic;
        var name1 = nameStatusUser;
        var path = __dirname + "/public/StatusPublic/" + name;
        var path1 = __dirname + "/public/StatusUser/" + name1;
        ...
        var Nms = ArrStatusSendServer.length + 1;
        DataStatus.map(async (dataEmit, index) => {
            var UserSendKey = {
                //	key: JSON.stringify(soOnMess + index),
                key: JSON.stringify(Nms + index),
                User: dataEmit.User, //thang send cho Userapp, va app dang la thang nhan nhung lai dat o this.state( la UsernameNguoiSend )
                StatusMe: dataEmit.StatusMe,
                imageBase64: dataEmit.imaBase64,
                pathIma: dataEmit.pathIma,
                //   UserNhan: '', //them vao de hien thi thoi
                //  messengerNhan: '',
            };
            //  console.log('.UserSendKey: ==0:::', UserSendKey);
            ArrStatusSendServer.push(UserSendKey);
            console.log('ArrStatusSendServer trong socket.on(connect)00000000000000',ArrStatusSendServer);
        });
        ...
        var M1 = JSON.stringify(ArrStatusSendServer);
        //  console.log('ArrMessSendServerMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMM' +
        //     '1111111111111111111111111111111111111111111111' +
        //     '111111111111111111111111111111111111111111111', M1);
        //   fs.writeFileSync(path, M1);
        //   fs.writeFileSync(path1, M1);

        var data = M1;
        fs.writeFile(path, data, (err) => {
            if (err) {
                console.log('save status err', err);
            }
            else {
                console.log('save status path : ', path);
            }
        });
        fs.writeFile(path1, data, (err) => {
            if (err) {
    ...
    socket.on('client-yeucau-status-public-congkhai', (dataYeuCau) => {
        ...
        var path = __dirname + "/public/StatusPublic/" + name;
        //  var path1 = __dirname + "/public/StatusUser/" + name1;
        var soPage = dataYeuCau.soPage;
        // var y = fs.readFileSync(path);
        // console.log('yyyyyyyyyyy', y);

        var ArrSocketIdUserYeuCauMess = []; // no la app hoac web can co no de biet tra ve chua socketId nao
        //ArraySocketIdUsername.push({ UserSocketId: socket.id + Username, Username: Username });
        var UserYeuCauMess = dataYeuCau.UserYeuStatus;
        ArraySocketIdUsername.map(function (value, index) {
            var UserSocketId = value.UserSocketId;
            if (UserSocketId.indexOf(UserYeuCauMess) > -1) {
                var socketId = UserSocketId.replace(UserYeuCauMess, '');
        ...
        fs.readFile(path, 'UTF-8', (err, data) => {
            if (err) {
                ...
                if (X <= 0) {
                    var X1 = 3;
                    ArrSocketIdUserYeuCauMess.map(function (SocketId, index) {
                        io1.to(SocketId).emit('server-trave-status-public-congKhai', { Nms: n, Sms: (SaveDataMessengerApp1), TH1: (n + ' <= ' + X1) });
                        // console.log('server-trave-yeucau-ArrayMess-User la : ', SaveDataMessengerApp);
                    });
                    // console.log('server-trave-Client-yeucau-ArrayMess-User la: ' + ReadArrayMessUsersendUserItem.NameUserSendUserItem + ":" +
                    //      '///////////////////////////////X <= 0/////////////////////////////////' +
                    ...
                    if (n <= X) {
                        ArrSocketIdUserYeuCauMess.map(function (SocketId, index) {
                            // io1.to(SocketId).emit('server-trave-yeucau-ArrayMess-User', { Nms: n, Sms: (SaveDataMessengerApp1), TH1: (n + ' <= ' + X) });
                            // console.log('server-trave-yeucau-ArrayMess-User la : ', SaveDataMessengerApp);
                            io1.sockets.emit('server-trave-status-public-congKhai', { Nms: n, Sms: (SaveDataMessengerApp1), TH1: (n + ' <= ' + X) })
                        });
                        // console.log('server-trave-status-public-congKhai la: ' + ReadArrayMessUsersendUserItem.NameUserSendUserItem + ":" +
                        //   console.log('server-trave-status-public-congKhai la: ' + name + ":" +
                        //     '////////////////////////////////////////////////////////////////' +
                        //      '//////////////////////////////////////////////////////////////', { Nms: n, Sms: SaveDataMessengerApp1, TH1: (n + ' <= ' + X) });
                    }
                    else if (n > X) {
                        ...
                        ArrSocketIdUserYeuCauMess.map(function (SocketId, index) {
                            // io1.to(SocketId).emit('server-trave-yeucau-ArrayMess-User', { Nms: n, Sms: (ArrXphanTuCuoiMs), TH2: (n + ' > ' + X) });
                            // console.log('server-trave-yeucau-ArrayMess-User la : ', SaveDataMessengerApp);
                            io1.sockets.emit('server-trave-status-public-congKhai', { Nms: n, Sms: (ArrXphanTuCuoiMs), TH2: (n + ' > ' + X) })
                        });
                        // console.log('server-trave-status-public-congKhai la: ' + ReadArrayMessUsersendUserItem.NameUserSendUserItem + ":" +
                        //   console.log('server-trave-status-public-congKhai la: ' + name + ":" +
                        //   '////////////////////////////////////////////////////////////////' +
    ...
    var ArrStatusUser = [];
    socket.on('client-yeucau-status-User', dataYeuCau => {


        console.log('client-share-status-public-congKhai:::', dataYeuCau);
        //  var UserClick = dataYeuCau.UserClick;
        var nameStatusUser = dataYeuCau.UserClick + ".docx";
        var name1 = nameStatusUser;
        ...
        fs.readFile(path1, 'UTF-8', (err, data) => {
            ...
            if (err) {
                ...
                ArrStatusUser = JSON.parse(data);
                console.log('ArrStatusSendServer trong socket.on(connect)', ArrStatusUser);
            }
        ...
        fs.readFile(path1, 'UTF-8', (err, data) => {
            if (err) {
                ...
                ArrSocketIdUserYeuCauMess.map(function (SocketId, index) {
                    // io1.to(SocketId).emit('server-trave-yeucau-ArrayMess-User', { Nms: 0, Sms: '0' });
                    // console.log('server-trave-yeucau-ArrayMess-User la : ', SaveDataMessengerApp);
                    io1.to(SocketId).emit('server-trave-status-public-congKhai', { Nms: 0, Sms: '0' });
...
app3.get('/ChatUser3/ReactChatUser12', (req, res) => {
    res.render('ChatUser3');
});

app3.get('/videocall',(req, res) => {
